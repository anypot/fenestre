...
- **Core (pure):** `WMState` (`src/state/wm.rs`) and the layout engine
  (`src/layout/`). They know nothing about River/Wayland. The single entry point
  is `WMState::handle_event(Event)`, a pure reducer over the domain `Event` enum
...
- Invariant 1 in `architecture.md` (Hexagonal boundary) is compiler-enforced for
  the core: `state/wm.rs` and `layout/` contain no `protocol::` imports and no
  River calls.
